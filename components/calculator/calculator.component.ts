...
export class CalculatorComponent implements OnInit {
    ...
    keyPress(value: any) {
        ...
        if (value === 'result' && this.operators.length) {
            ...
            if (value === 0 || this.isNumeric(value) || (!this.isNumeric(value) && this.isNumeric(this.lastOperator()) && this.lastOperator() !== '.')) {
    ...
    isNumeric(value) {
        if (value) {
            return (parseInt(value, 10) > 0) || value === '0' || value === 0 || value === '.' || value === '(' || value === ')';
        }
        return false;
    }
    ...
    renderResult() {
        ...
        if (this.operators.length === 1 && !this.isNumeric(this.lastOperator()) && this.lastOperator() !== 0) {
            this.operators = [];
            this.output = '0';
        }
        ...
        for (const operator of this.operators) {
            ...
            const number = this.isNumeric(operator) || operator === 0;
